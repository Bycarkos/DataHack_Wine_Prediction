...
def visualize_predictions(df_response: pd.DataFrame, metaclass:Optional[List] = None):
...
def write_verbose(df, metaclass, output_path="output.csv"):
    ...
    if groupby_columns:
        ...
        for name, group in groups:
            group_results = get_metrics(group)
            group_results.insert(0, name)
            results.loc[len(results)] = group_results
    else:
        results = pd.DataFrame(columns=["RMSE", "R2", "error_percent"])
        results.loc[0] = get_metrics(df)

    results.to_csv(output_path, index=False)

    
def compare_metric_to_beat(df: pd.DataFrame, rmse_path: str = "metric_to_beat.csv"):
    ...
    if not os.path.exists(rmse_path):
        write_metric_to_beat(rmse=rmse)
...
def write_response_file(x:pd.DataFrame, predictions:np.ndarray, name_file:Optional[Path]):
...
def write_interval_confidence(df_response, metaclass):
    pass

def write_model_decission_confidence(df_response, metaclasses):
    pass
